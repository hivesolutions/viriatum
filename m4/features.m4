...
AC_ARG_ENABLE([http2], [AS_HELP_STRING([--disable-http2], [disable http2 support])], [test "x$enableval" = xno && have_http2=false], [])
...
if test "$have_http2" = false; then
    AC_DEFINE(HAVE_NO_HTTP2, 1, [Define to 1 if http2 is disabled])
fi
