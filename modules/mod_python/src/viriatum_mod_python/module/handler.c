...
ERROR_CODE create_mod_python_http_handler(struct mod_python_http_handler_t **mod_python_http_handler_pointer, struct http_handler_t *http_handler) {
    ...
    mod_python_http_handler->module = NULL;
...
ERROR_CODE delete_mod_python_http_handler(struct mod_python_http_handler_t *mod_python_http_handler) {
    ...
    size_t index;
    struct mod_python_location_t *location;
    ...
    for(index = 0; index < mod_python_http_handler->locations_count; index++) {
        location = &mod_python_http_handler->locations[index];
    ...
    if(mod_python_http_handler->locations != NULL) { FREE(mod_python_http_handler->locations); }

    /* in case the execution module is defined for the WSGI handler
    it must be released by decrementing the reference count */
    if(mod_python_http_handler->module != NULL) { Py_DECREF(mod_python_http_handler->module); }
...
ERROR_CODE set_handler_wsgi(struct http_connection_t *http_connection) {
    ...
    _set_http_request_handler_wsgi(http_connection->request);
...
ERROR_CODE unset_handler_wsgi(struct http_connection_t *http_connection) {
    ...
    _unset_http_request_handler_wsgi(http_connection->request);
...
ERROR_CODE url_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
    /* retrieves the handler WSGI context from the HTTP parser */
    struct handler_wsgi_context_t *handler_wsgi_context = (struct handler_wsgi_context_t *) http_request->context;
...
ERROR_CODE header_field_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
    /* retrieves the handler WSGI context from the HTTP parser */
    struct handler_wsgi_context_t *handler_wsgi_context = (struct handler_wsgi_context_t *) http_request->context;
...
ERROR_CODE header_value_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
    /* retrieves the handler WSGI context from the HTTP parser */
    struct handler_wsgi_context_t *handler_wsgi_context = (struct handler_wsgi_context_t *) http_request->context;
...
ERROR_CODE headers_complete_callback_handler_wsgi(struct http_request_t *http_request) {
    /* raise no error */
    RAISE_NO_ERROR;
}

ERROR_CODE body_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
    /* raise no error */
    RAISE_NO_ERROR;
}

ERROR_CODE message_complete_callback_handler_wsgi(struct http_request_t *http_request) {
    ...
    return_value = _send_response_handler_wsgi(http_request);
    if(IS_ERROR_CODE(return_value)) {
        ...
        struct handler_wsgi_context_t *handler_wsgi_context =
            (struct handler_wsgi_context_t *) http_request->context;
        V_WARNING_CTX_F("mod_python", "WSGI error for %s: %s\n", handler_wsgi_context ? (char *) handler_wsgi_context->url : "/", (char *) GET_ERROR());
        _write_error_connection_wsgi(http_request, (char *) GET_ERROR());
    }
...
ERROR_CODE path_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
    /* raise no error */
    RAISE_NO_ERROR;
}

ERROR_CODE location_callback_handler_wsgi(struct http_request_t *http_request, size_t index, size_t offset) {
    ...
    struct mod_python_http_handler_t *mod_python_http_handler =
        (struct mod_python_http_handler_t *) http_connection->http_handler->lower;

    /* retrieves the current location from the location buffer and checks if the
    file path value is correctly set */
    struct mod_python_location_t *location = &mod_python_http_handler->locations[index];
...
ERROR_CODE virtual_url_callback_handler_wsgi(struct http_request_t *http_request, const unsigned char *data, size_t data_size) {
...
ERROR_CODE _set_http_request_handler_wsgi(struct http_request_t *http_request) {
...
ERROR_CODE _unset_http_request_handler_wsgi(struct http_request_t *http_request) {
    /* retrieves the handler WSGI context from the HTTP parser
...
ERROR_CODE _send_response_handler_wsgi(struct http_request_t *http_request) {
    ...
    struct http_connection_t *http_connection = (struct http_connection_t *) io_connection->lower;
    struct handler_wsgi_context_t *handler_wsgi_context = (struct handler_wsgi_context_t *) http_request->context;
    struct mod_python_http_handler_t *mod_python_http_handler = (struct mod_python_http_handler_t *) http_connection->http_handler->lower;
    ...
    handler_wsgi_context->reload = handler_wsgi_context->reload == UNSET ? mod_python_http_handler->reload : handler_wsgi_context->reload;
    handler_wsgi_context->module = handler_wsgi_context->module_pointer == NULL ? mod_python_http_handler->module : handler_wsgi_context->module;
    handler_wsgi_context->module_pointer = handler_wsgi_context->module_pointer == NULL ? &mod_python_http_handler->module : handler_wsgi_context->module_pointer;
    handler_wsgi_context->module_name = handler_wsgi_context->module_name == NULL ? mod_python_http_handler->module_name : handler_wsgi_context->module_name;
    ...
    if(handler_wsgi_context->module == NULL) {
        ...
        file_path = mod_python_http_handler->file_path[0] == '\0'
                        ? DEFAULT_FILE_PATH
                        : mod_python_http_handler->file_path;
        ...
        SPRINTF(
            ...
            "wsgi_app_%lu",
            mod_python_http_handler->counter
        );
        mod_python_http_handler->counter++;
    ...
    handler_wsgi_context->flags = http_request->flags;
    ...
    _start_environ_wsgi(environ, http_request);
...
ERROR_CODE _write_error_connection_wsgi(struct http_request_t *http_request, char *message) {
...
ERROR_CODE _start_environ_wsgi(PyObject *environ, struct http_request_t *http_request) {
...
ERROR_CODE _load_module_python(PyObject **module_pointer, char *name, char *file_path) {
    ...
    *module_pointer = NULL;
    ...
    if(code == NULL) {
        V_DEBUG_CTX("mod_python", "Error while compiling module\n");
        PyErr_Clear();
        RAISE_NO_ERROR;
    }
    ...
    *module_pointer = module;
    ...
    if(module == NULL) {
        V_DEBUG_CTX("mod_python", "Error while executing module\n");
        PyErr_Clear();
    }
