...
int main(int argc, char **argv) {
    ...
    V_PRINT_F(
        "viriatum_mod_asgi test runner (%s, %s)\n",
        VIRIATUM_COMPILATION_DATE,
        VIRIATUM_COMPILATION_TIME
    );

    /* initializes the python interpreter so that the loading of an
    application runs against a fully initialized virtual machine */
    Py_Initialize();

    return_value = run_mod_asgi_tests();
