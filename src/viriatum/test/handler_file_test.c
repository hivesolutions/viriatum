...
const char *test_handler_file_through(void) {
    /* allocates space for the chain of the connection, for the
    message being served and for the response it produces */
    struct test_context_t *context;
    struct http_request_t *http_request;
    ...
    /* asks for a path that continues through the file as though it
    were a directory, the opening of it is refused for a reason that
    says neither that the path is a directory nor that it is not there,
    which is what sends the handler to describe the path and find
    nothing it is able to serve, whoever the process runs as */
    error = url_callback_handler_file(
        http_request,
        (unsigned char *) "/" HANDLER_FILE_TEST_NAME "/inner.txt",
        sizeof(HANDLER_FILE_TEST_NAME) + 10
    );
    V_ASSERT(error == 0);
    error = message_complete_callback_handler_file(http_request);
    V_ASSERT(error == 0);
    ...
    /* a path that leads through a file is answered the way one that
    is not there is, and never as the listing of a directory */
    V_ASSERT(strstr((char *) written, "HTTP/1.1 404 Not Found\r\n") == (char *) written);
    V_ASSERT_EQ_U(get_closed_test_connection(), 0);
